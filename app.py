...
ticker_symbol = st.sidebar.text_input("輸入股票代號 (如: COHR, NVDA, TSM)", value="COHR").upper()
...
def load_and_process_data(ticker, period, interval):
    ...
    if not df.empty:
        ...
        if len(df) >= 200:
            ...
            df['MA200_Past'] = df['200MA'].shift(20) # 抓取約一個月前的位置比對趨勢
        else:
            df['200MA'] = None
            df['MA200_Past'] = None
...
if not daily_data.empty:
    ...
    col4.metric("趨勢指標得分", f"{pass_count} / 7", help="滿分7分代表完全符合 Mark Minervini 第二階段上升趨勢模板")

    # --- 趨勢健檢清單 (預設收起，保持介面如同看盤軟體般俐落) ---
    with st.expander("📊 點此展開 Minervini 趨勢模板詳細健檢狀態", expanded=False):
        ...
        with col_res:
            if pass_count == 7:
                ...
                st.info(f"蓄勢待發：已滿足 {pass_count} 項條件，可密切關注均線與成交量收縮狀況。")
            else:
                st.warning(f"目前結構偏弱或處於深度調整，僅符合 {pass_count} 項條件。")
    ...
    def render_futu_chart(ticker, period_str, interval_str):
        ...
        if not data.empty:
            # 為了畫面比例舒適，日K預設截取近一年的K線展示；周K與月K則展示完整波段
            df_plot = data.tail(252) if interval_str == "1d" else data
            
            fig = make_subplots(
                rows=2, cols=1, shared_xaxes=True, 
                vertical_spacing=0.02, 
                row_width=[0.25, 0.75] # 上下圖表高度黃金比例 3:1
            )

            # 1. 繪製精緻 K 線圖 (使用貼近牛牛介面的明亮紅綠配色)
            fig.add_trace(
                go.Candlestick(
            ...
            if 'Vol_50MA' in df_plot.columns and df_plot['Vol_50MA'].notna().any():
                fig.add_trace(
                    go.Scatter(
                        x=df_plot.index, y=df_plot['Vol_50MA'], name='50均量', 
                        line=dict(color='rgba(255, 165, 0, 0.65)', width=1.5)
                    ), 
                    row=2, col=1
                )

            # 完美套用游標連動手感 (x unified) 與極致黑底介面
            fig.update_layout(
                height=680, 
                xaxis_rangeslider_visible=False, 
                template="plotly_dark",
                hovermode="x unified", # 游標經過時，自動產生垂直線並整合顯示當日完整數據
                hoverlabel=dict(bgcolor="#1e1e1e", font_size=13, font_color="#ffffff"),
